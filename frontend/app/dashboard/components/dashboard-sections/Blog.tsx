...
const PageWidget = ({ title, icon: Icon, children, action }: any) => (
  <div className="relative z-10 w-full bg-[#111111] border border-[#222] rounded-xl flex flex-col overflow-visible shadow-[0_15px_30px_-10px_rgba(0,0,0,0.5)] hover:border-[#333] transition-colors h-[600px]">
    <div className="px-5 py-4 border-b border-[#222] flex items-center justify-between bg-[#141414] rounded-t-xl shrink-0">
    ...
    <div className="flex-1 bg-[#111111] min-h-0 relative flex flex-col rounded-b-xl text-neutral-300 overflow-hidden">
      {children}
    </div>
  </div>
);
    ...
    <div className="space-y-8 font-sans">
        ...
        action={
          <div className="relative" ref={filterRef}>
            <button 
              ...
              className={`p-1.5 rounded-md transition-colors ${isFilterOpen ? 'bg-[#222] text-white' : 'text-neutral-500 hover:text-neutral-300 hover:bg-[#1a1a1a]'}`}
            >
              <Filter size={14} />
            </button>
            ...
            {/* Dropdown Menu */}
            {isFilterOpen && (
              <div className="absolute right-0 top-full mt-2 w-48 bg-[#1a1a1a] border border-[#333] rounded-xl shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                <div className="px-3 py-2 border-b border-[#2a2a2a] text-[10px] font-semibold text-neutral-500 uppercase tracking-wider">
                  Sort by
                </div>
                ...
                <button 
                  ...
                  className="w-full text-left px-4 py-2.5 text-xs text-neutral-300 hover:bg-[#222] hover:text-white transition-colors flex items-center justify-between group"
                >
            ...
            {sortedUpdates.map((update) => {
               ...
               return (
                <div 
                  ...
                  className="group border-b border-[#1a1a1a] p-4 hover:bg-[#161616] cursor-pointer transition-all duration-200 flex flex-col gap-2 relative"
                >
                  <div className="flex items-start justify-between w-full">
                     <div className="flex items-center gap-3">
                        {/* Avatar in List */}
                        <div className="w-8 h-8 rounded-full bg-[#222] border border-[#333] flex items-center justify-center overflow-hidden shrink-0">
                           {avatarUrl ? (
                  ...
                  <div className="pl-11 flex items-center gap-2 mt-1">
                     {update.version && (
                       <span className="text-[10px] px-1.5 py-0.5 rounded border border-[#2a2a2a] bg-[#1a1a1a] text-neutral-400 font-mono">
                         v{update.version}
                       </span>
                     )}
                     {update.tag && update.tag.tag && (
                       <span 
                          className="text-[10px] px-1.5 py-0.5 rounded border border-[#2a2a2a] bg-[#1a1a1a] flex items-center gap-1"
      ...
      {selectedUpdate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative w-full max-w-2xl bg-[#0f0f0f] border border-[#222] rounded-2xl shadow-2xl flex flex-col max-h-[85vh] animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-start justify-between p-6 border-b border-[#1f1f1f]">
              <div className="flex items-start gap-4">
                 <div className="w-12 h-12 rounded-full bg-[#1a1a1a] border border-[#333] flex items-center justify-center overflow-hidden shrink-0 mt-1">
                 ...
                 <div className="space-y-1">
                    ...
                    {selectedUpdate.tag && selectedUpdate.tag.tag && (
                       <div className="pt-2">
                         <span 
                            className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md border border-[#2a2a2a] bg-[#161616] text-xs"
              ...
              <button 
                onClick={() => setSelectedUpdate(null)}
                className="p-2 rounded-lg bg-[#1a1a1a] hover:bg-[#222] text-neutral-400 hover:text-white transition-colors border border-[#2a2a2a]"
              >
                <X size={18} />
              </button>
            </div>

            <div className="p-8 overflow-y-auto text-neutral-300 leading-relaxed space-y-4">
            ...
            <div className="p-4 border-t border-[#1f1f1f] bg-[#111111] rounded-b-2xl flex justify-end">
              <button 
                onClick={() => setSelectedUpdate(null)}
                className="px-4 py-2 text-sm font-medium text-neutral-300 bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg hover:bg-[#222] hover:text-white transition-colors"
              >
                Close
              </button>
            </div>
