...
export default function RoleManagementModal({ 
  ...
  const allPermissions: Permission[] = [
    // OVERVIEW section
    { id: "dashboard", name: "Dashboard", category: "Overview", description: "View project dashboard and overview", icon: <LayoutDashboard size={14} className="text-white" /> },
    
    // DEVELOPMENT section
    { id: "tasks", name: "Tasks", category: "Development", description: "Create, edit, and manage project tasks", icon: <ListTodo size={14} className="text-white" /> },
    { id: "board", name: "Board", category: "Development", description: "Access and manage project board", icon: <LayoutDashboard size={14} className="text-white" /> },
    { id: "events-workflow", name: "Events & Workflow", category: "Development", description: "Manage events and workflow automation", icon: <Workflow size={14} className="text-white" /> },
    { id: "activity-overview", name: "Activity Overview", category: "Development", description: "View project activity and overview", icon: <Activity size={14} className="text-white" /> },
    { id: "repository-logs", name: "Repository Logs", category: "Development", description: "Access repository logs and history", icon: <GitBranch size={14} className="text-white" /> },
    
    // ARTIFICIAL INTELLIGENCE section
    { id: "ai-assistant", name: "AI Assistant", category: "AI Tools", description: "Access AI assistant features", icon: <Bot size={14} className="text-white" /> },
    { id: "ai-repo-review", name: "AI Repo Review", category: "AI Tools", description: "Use AI for repository code review", icon: <BookOpen size={14} className="text-white" /> },
    { id: "ai-sql-helper", name: "AI SQL Helper", category: "AI Tools", description: "Get AI assistance with SQL queries", icon: <Database size={14} className="text-white" /> },
    { id: "ai-roadmap-visualizer", name: "AI Roadmap Visualizer", category: "AI Tools", description: "Visualize project roadmap with AI", icon: <Map size={14} className="text-white" /> },
    
    // SETTINGS section
    { id: "project-settings", name: "Project Settings", category: "Settings", description: "Configure project settings", icon: <Settings size={14} className="text-white" /> },
    { id: "collaborators", name: "Collaborators", category: "Settings", description: "Manage project collaborators", icon: <UserPlus size={14} className="text-white" /> },
    { id: "ai-monitor", name: "AI Monitor", category: "AI Tools", description: "Monitor project with AI insights", icon: <Monitor size={14} className="text-white" /> },
    
    // MANAGER SETTINGS section
    { id: "manager-project-settings", name: "Project Settings (Manager)", category: "Manager Settings", description: "Manager-level project configuration", icon: <Settings size={14} className="text-white" /> },
    { id: "manager-workflow-events", name: "Workflow and Events (Manager)", category: "Manager Settings", description: "Manager-level workflow and event management", icon: <Workflow size={14} className="text-white" /> },
    { id: "management", name: "Management", category: "Manager Settings", description: "General management capabilities", icon: <Briefcase size={14} className="text-white" /> },
    { id: "manage-team", name: "Manage Team", category: "Manager Settings", description: "Team management and assignments", icon: <UserPlus size={14} className="text-white" /> },
    { id: "manage-ai-assistant", name: "Manage AI Assistant", category: "Manager Settings", description: "AI Assistant configuration and management", icon: <Bot size={14} className="text-white" /> },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-[#18181b] border border-zinc-800 rounded-xl shadow-2xl w-full max-w-6xl p-6 relative animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
<style jsx global>{`
...
`}</style>
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-lg font-bold text-white">Manage Project Roles</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <div className="flex-1 relative">
                  ...
                  <input 
                    ...
                    className="w-full bg-zinc-900/50 border border-zinc-800/80 rounded-lg pl-9 pr-4 py-2 text-sm text-zinc-200 focus:outline-none focus:ring-1 focus:ring-zinc-700 focus:border-zinc-700 transition-all" 
                    value={searchQuery}
            ...
            <div className="space-y-2">
              {filteredRoles.map(role => {
                ...
                return (
                  <div key={role.id} className="bg-zinc-900/30 border border-zinc-800/50 rounded-lg overflow-hidden">
                    {/* Role Row */}
                    <div 
                      onClick={() => toggleRoleExpand(role.id)}
                      className="grid grid-cols-12 gap-4 px-4 py-3 items-center transition-all cursor-pointer hover:bg-zinc-800/20"
                    >
                      {/* Role Details */}
                      ...
                      <div className="col-span-2 flex items-center justify-end gap-2 text-right">
                        ...
                        <button className={`p-1.5 transition-colors ${isExpanded ? 'text-zinc-100' : 'text-zinc-600 hover:text-zinc-300'}`}>
                    ...
                    {isExpanded && (
                      <div className="border-t border-zinc-800 bg-zinc-900/20 px-4 py-4 animate-in slide-in-from-top-2 duration-200">
                        <div className="flex flex-wrap gap-1.5">
                          {role.permissions.map(permId => {
                            ...
                            return perm ? (
                              <span 
                                key={perm.id} 
                                className="px-2 py-1 rounded bg-zinc-800/40 border border-zinc-800 text-[10px] text-zinc-400 font-mono flex items-center gap-1"
                              >
                                <span className="text-white">{perm.icon}</span>
                                {perm.name}
                              </span>
                            ) : (
                              <span 
                                key={permId} 
                                className="px-2 py-1 rounded bg-zinc-800/40 border border-zinc-800 text-[10px] text-zinc-400 font-mono"
                              >
                                {permId}
                              </span>
                            );
          ...
          <div className="lg:col-span-1">
            {(isCreating || editingRole) ? (
              <div className="bg-zinc-900/30 border border-zinc-800/50 rounded-lg p-5 sticky top-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-zinc-900/50 border border-zinc-800/80 rounded-lg px-3 py-2 text-sm text-zinc-200 focus:outline-none focus:ring-1 focus:ring-zinc-700 focus:border-zinc-700 transition-all"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-zinc-900/50 border border-zinc-800/80 rounded-lg px-3 py-2 text-sm text-zinc-200 focus:outline-none focus:ring-1 focus:ring-zinc-700 focus:border-zinc-700 transition-all resize-none"
                    />
                  </div>
                            ...
                            <div className="flex items-start gap-2">
                              ...
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-0.5">
                                  ...
                                  <span className="text-[10px] text-zinc-500 bg-zinc-800 px-1.5 py-0.5 rounded-full ml-auto">
                                    {permission.category}
                                  </span>
                                </div>
              ...
              <div className="bg-zinc-900/30 border border-zinc-800/50 rounded-lg p-5 sticky top-6">
                <h3 className="text-sm font-semibold text-white mb-4">Available Permissions</h3>
                <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                  {allPermissions.map(permission => (
                    <div key={permission.id} className="p-2 border border-zinc-800 rounded-md">
                      ...
                      <p className="text-[10px] text-zinc-400">{permission.description}</p>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="mt-6 pt-6 border-t border-zinc-800 flex justify-between items-center">
          ...
          <div className="flex gap-3">
            <button
              onClick={onClose}
              className="px-4 py-2 border border-zinc-700 hover:bg-zinc-800 text-zinc-300 rounded-lg transition-colors text-sm"
            >
              Cancel
            </button>
            <button
