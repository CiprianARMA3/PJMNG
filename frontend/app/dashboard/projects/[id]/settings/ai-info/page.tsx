...
export default function AIUsagePage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // 1. Calculate Date Ranges (Current Week vs Previous Week)
            const now = new Date();
            ...
            // 2. Fetch Data
            // Note: For trend calculation, we need logs from at least 2 weeks ago
            const [logsResponse, packResponse] = await Promise.all([
                supabase
                    .from('token_usage_logs')
                    .select('*')
                    .eq('project_id', id)
                    .gte('created_at', startOfPrevWeek.toISOString()) // Get from 2 weeks ago
            ...
            setLogs(rawLogs); // Note: This contains 2 weeks of logs now

            // 3. Process Users (Unique IDs from logs)
    ...
    return (
        <div className="h-screen bg-[#0a0a0a] light:bg-gray-50 text-zinc-100 light:text-black flex overflow-hidden selection:bg-indigo-500/30">
            ...
            <main className="flex-1 ml-64 flex flex-col h-full bg-[#09090b] light:bg-white relative">
                ...
                <div className="flex-none h-14 mt-[55px] px-6 border-b border-white/5 light:border-gray-200 flex items-center justify-between bg-[#0a0a0a]/80 light:bg-white/80 backdrop-blur-md z-20">
                    <div className="flex items-center gap-4">
                        <h1 className="text-xl font-bold tracking-tight flex items-center gap-2">
                            <Brain size={20} className="text-indigo-500" />
                            AI <span className="text-white/30 light:text-black/30 text-lg font-light">Monitor</span>
                        </h1>
                    </div>
                    <div className="flex items-center gap-3">
                        <div className="h-6 px-2.5 bg-indigo-500/10 light:bg-indigo-50 border border-indigo-500/20 light:border-indigo-200 rounded-full text-[10px] font-bold text-indigo-400 light:text-indigo-700 flex items-center gap-2">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 px-6 py-6 animate-in fade-in slide-in-from-bottom-3 duration-500">
                        ...
                        <div className="bg-[#0C0C0E] light:bg-white border border-zinc-800 light:border-gray-200 p-5 rounded-xl relative overflow-hidden group/credits z-10 transition-shadow duration-300   hover:shadow-lg">
                            ...
                            <div className="text-4xl font-mono font-bold text-white light:text-black mb-2 relative inline-block">
                            ...
                            <p className="text-zinc-400 light:text-gray-600 text-xs mt-4">For further informations about the AI usage and consumption consult &lt;Token Usage and Consumption&gt;. The amount of tokens is dependend of the project's Administrator.</p>

                        </div>

                        {/* Card 2: USAGE & TREND */}
                        <div className="bg-[#0C0C0E] light:bg-white border border-zinc-800 light:border-gray-200 p-5 rounded-xl relative overflow-hidden group transition-shadow duration-300   hover:shadow-lg">
                            ...
                            <div className="text-4xl font-mono font-bold text-white light:text-black mb-2 flex items-center gap-3">
                            ...
                            <div className="flex items-center gap-2 text-md text-zinc-400 light:text-gray-600">
                                <span>Previous week</span>
                                <span className="text-zinc-600 light:text-gray-400">|</span>
                                <span>Avg: <span className="text-zinc-200 light:text-black">{dailyAverage.toFixed(0)}</span> / day</span>
                            </div>
                            <p className="text-zinc-400 light:text-gray-600 text-xs mt-4">The weekly consumption doesn't affect the project's billing nor will it ever get limitations based on the token's usage.</p>
                        </div>

                        {/* Card 3: MODEL DISTRIBUTION (Detailed List) */}
                        <div className="bg-[#0C0C0E] light:bg-white border border-zinc-800 light:border-gray-200 p-5 rounded-xl relative overflow-hidden group transition-shadow duration-300   hover:shadow-lg flex flex-col">
                            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                                <LayoutGrid size={35} />
                            </div>
                            ...
                            <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 -mr-1 space-y-3">
                                {Object.keys(tokenPack).length > 0 ? (
                                    Object.entries(tokenPack)
                                        ...
                                        .map(([key, value], index) => {
                                            ...
                                            return (
                                                ...
                                                <div
                                                    key={key}
                                                    className="flex items-center justify-between group/item transition-all duration-200 hover:bg-zinc-900/40 light:hover:bg-gray-100 p-1 rounded-lg -mx-1"
                                                    style={{ animationDelay: `${index * 50}ms` }} // Staggered list entry (optional but nice)
                                                >
                                                    <div className="flex items-center gap-3 min-w-0">
                                                        {/* Icon Box */}
                                                        <div className="w-8 h-8 rounded-lg bg-zinc-900 light:bg-gray-100 border border-zinc-800 light:border-gray-200 flex items-center justify-center text-zinc-500 light:text-gray-500 group-hover/item:text-indigo-400 group-hover/item:border-indigo-500/30 transition-all duration-300">
                                                            <Icon size={14} />
                                                        </div>

                                                        {/* Name & Desc */}
                                                        <div className="flex flex-col min-w-0">
                                                            <span className="text-xs font-medium text-zinc-200 light:text-black truncate group-hover/item:text-white light:group-hover/item:text-black transition-colors">
                                                                {modelConfig.name}
                                                            </span>
                                    ...
                                    <div className="h-full flex flex-col items-center justify-center text-xs text-zinc-600 light:text-gray-500 italic gap-2">
                    ...
                    <div className="px-6 pb-6 animate-in fade-in slide-in-from-bottom-3 duration-500 delay-100">
                        <div className="h-[280px] w-full bg-[#0C0C0E] light:bg-white border border-zinc-800 light:border-gray-200 rounded-xl p-4 relative transition-shadow duration-300   hover:shadow-lg">
                            <h3 className="text-xs font-semibold text-zinc-400 light:text-gray-600 mb-4 flex items-center gap-2">
                    ...
                    <div className="sticky top-0 bg-[#09090b]/95 light:bg-white/95 backdrop-blur-sm z-10 px-6 py-4 border-y border-zinc-800/50 light:border-gray-200 flex items-center justify-between">
                        <div className="flex items-center gap-3 w-full max-w-2xl">
                            <div className="relative flex-1 group">
                                <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500 light:text-gray-500 group-focus-within:text-zinc-300 light:group-focus-within:text-gray-700 transition-colors" />
                                <input
                                    type="text"
                                    placeholder="Search Request ID, Model, or User..."
                                    className="w-full bg-zinc-900/50 light:bg-gray-100 border border-zinc-800/80 light:border-gray-200 rounded-lg pl-9 pr-4 py-2 text-sm text-zinc-200 light:text-black placeholder:text-zinc-600 light:placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-zinc-700 light:focus:ring-gray-300 focus:border-zinc-700 light:focus:border-gray-300 transition-all shadow-sm font-mono"
                                    value={searchQuery}
                            ...
                            <div className="relative">
                                ...
                                ${showModelMenu
                                            ...
                                            : 'bg-zinc-900/50 light:bg-gray-100 border-zinc-800 light:border-gray-200 text-zinc-400 light:text-gray-600 hover:text-zinc-200 light:hover:text-black hover:bg-zinc-900 light:hover:bg-gray-200'}`}
                                >
                                    <Filter size={14} />
                                ...
                                {showModelMenu && (
                                    <div className="absolute top-full right-0 mt-2 w-56 bg-[#0C0C0E] light:bg-white border border-zinc-800 light:border-gray-200 rounded-lg shadow-2xl z-30 p-1.5 animate-in fade-in zoom-in-95 duration-200">
                                        <button
                                            onClick={() => { setModelFilter("All Models"); setShowModelMenu(false); }}
                                            className={`w-full text-left px-2 py-1.5 text-xs rounded-md transition-colors ${modelFilter === "All Models" ? 'bg-zinc-800 light:bg-gray-100 text-white light:text-black' : 'text-zinc-400 light:text-gray-600 hover:bg-zinc-800/50 light:hover:bg-gray-100'}`}
                                        >
                                            All Models
                                        </button>
                                        {uniqueModels.map(m => (
                                            <button
                                                ...
                                                className={`w-full text-left px-2 py-1.5 text-xs rounded-md transition-colors truncate ${modelFilter === m ? 'bg-zinc-800 light:bg-gray-100 text-white light:text-black' : 'text-zinc-400 light:text-gray-600 hover:bg-zinc-800/50 light:hover:bg-gray-100'}`}
                                            >
                                                {formatModelName(m)}
                                            </button>
                                        ))}
                    ...
                    <div className="min-h-[400px]">
                        <div className="grid grid-cols-12 gap-4 px-6 py-2 border-b border-zinc-800/50 light:border-gray-200 text-[10px] font-semibold uppercase tracking-wider text-zinc-500 light:text-gray-500 select-none bg-zinc-900/20 light:bg-gray-50">
                        ...
                        <div className="pb-10">
                                    ...
                                    >
                                        ...
                                        <div className="col-span-3 flex items-center gap-3">
                                            ...
                                            <div className="flex flex-col overflow-hidden">
                                                <span className="text-xs font-medium text-zinc-300 light:text-black truncate">
                                        ...
                                        <div className="col-span-2">
                                            <span className="inline-flex items-center px-1.5 py-0.5 rounded border border-zinc-800 light:border-gray-200 bg-zinc-900/50 light:bg-gray-100 text-[10px] font-medium text-zinc-300 light:text-black">
                                        ...
                                        <div className="col-span-2 text-right">
                                            <span className="text-xs font-mono font-medium text-zinc-200 light:text-black">
                            ...
                            {filteredLogs.length === 0 && (
                                <div className="flex flex-col items-center justify-center h-64 text-zinc-600 light:text-gray-500 animate-in fade-in zoom-in-95 duration-500">
