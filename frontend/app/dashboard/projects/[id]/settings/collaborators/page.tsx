...
export default function ProjectMembersPage() {
    ...
    return (
        <div className="h-screen bg-[#0a0a0a] light:bg-gray-50 text-zinc-100 light:text-black flex overflow-hidden selection:bg-indigo-500/30">

            {/* SIDEBAR */}
            <Menu project={project} user={user} />

            <main className="flex-1 ml-64 flex flex-col h-full bg-[#09090b] light:bg-white relative">
                {/* #262626  #09090b] */}
                ...
                <div className="flex-none h-14 mt-[55px] px-6 border-b border-white/5 light:border-gray-200 flex items-center justify-between bg-[#0a0a0a]/50 light:bg-white/80 backdrop-blur-sm z-10">
                    <div className="flex items-center gap-4">
                        <h1 className="text-xl font-bold tracking-tight">Collaborators <span className="text-white/30 light:text-black/30 text-lg font-light">Panel</span></h1>
                    </div>
                    <div className="flex items-center gap-3">
                        <div className="h-6 px-2 bg-zinc-900 light:bg-gray-100 border border-zinc-800 light:border-gray-200 rounded text-[12px] font-mono text-zinc-500 light:text-gray-500 flex items-center gap-2">
                            <span>SHOWING:</span>
                            <span className="text-zinc-300 light:text-black">{filteredMembers.length}</span>
                ...
                <div className="flex-none px-6 py-5 flex items-center justify-between">
                    <div className="flex items-center gap-3 w-full max-w-3xl">
                        ...
                        <div className="relative flex-1 group">
                            <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500 group-focus-within:text-zinc-300 light:group-focus-within:text-black transition-colors" />
                            <input
                                type="text"
                                placeholder="Search by name, email, or user ID..."
                                className="w-full bg-zinc-900/50 light:bg-white border border-zinc-800/80 light:border-gray-200 rounded-lg pl-9 pr-4 py-2 text-sm text-zinc-200 light:text-black placeholder:text-zinc-600 light:placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-zinc-700 light:focus:ring-gray-300 focus:border-zinc-700 light:focus:border-gray-300 transition-all shadow-sm"
                                value={searchQuery}
                        ...
                        <div className="relative">
                            ...
                            ${showRoleMenu
                                        ? 'bg-zinc-800 light:bg-gray-200 border-zinc-700 light:border-gray-300 text-white light:text-black'
                                        : 'bg-zinc-900/50 light:bg-white border-zinc-800 light:border-gray-200 text-zinc-400 light:text-gray-600 hover:text-zinc-200 light:hover:text-black hover:bg-zinc-900 light:hover:bg-gray-100'}`}
                            >
                                <Filter size={14} />
                            ...
                            {showRoleMenu && (
                                <div className="absolute top-full right-0 mt-2 w-48 bg-[#0C0C0E] light:bg-white border border-zinc-800 light:border-gray-200 rounded-lg shadow-2xl z-30 p-1.5 animate-in fade-in zoom-in-95 duration-100">
                                    <div className="space-y-0.5">
                                        <div className="px-2 py-1.5 text-[10px] font-bold text-zinc-500 light:text-gray-500 uppercase tracking-wider">Select Roles</div>
                                        {roleFilters.map(filter => (
                                            <button
                                                key={filter.name}
                                                onClick={() => toggleRoleFilter(filter.name)}
                                                className="w-full text-left flex items-center justify-between px-2 py-1.5 text-xs text-zinc-400 light:text-gray-600 hover:text-zinc-100 light:hover:text-black hover:bg-zinc-800/50 light:hover:bg-gray-100 rounded-md transition-colors"
                                            >
                        ...
                        <button
                            ...
                            className="p-2 bg-zinc-900/50 light:bg-white border border-zinc-800 light:border-gray-200 rounded-lg text-zinc-500 light:text-gray-500 hover:text-zinc-200 light:hover:text-black hover:bg-zinc-900 light:hover:bg-gray-100 hover:border-zinc-700 light:hover:border-gray-300 transition-all active:scale-95"
                            title="Refresh Data"
                        >
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    {filteredMembers.map(member => {
                        ...
                        return (
                            ...
                            ${isExpanded ? 'bg-zinc-900/30 light:bg-gray-100 border-zinc-800 light:border-gray-300' : 'hover:bg-zinc-900/20 light:hover:bg-gray-50'}`}
                                >
                                    {/* 1. Member Identity */}
                                    <div className="col-span-4 flex items-center gap-3">
                                        ...
                                        <div className="flex flex-col">
                                            <span className={`text-sm font-medium transition-colors ${isExpanded ? 'text-indigo-400' : 'text-zinc-200 light:text-black group-hover:text-white light:group-hover:text-black'}`}>
                                                {member.full_name}
                                            </span>
                                            <span className="text-xs text-zinc-500 light:text-gray-500 font-mono tracking-tight truncate max-w-[180px]">
                                                {member.email}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="col-span-2">
                                        <div className="inline-flex items-center px-2 py-0.5 rounded border border-zinc-800 light:border-gray-200 bg-zinc-900/50 light:bg-white">
                                            <div className={`w-1.5 h-1.5 rounded-full mr-2 ${member.role.toLowerCase().includes('admin') ? 'bg-rose-500' :
                                                    member.role.toLowerCase().includes('editor') ? 'bg-indigo-500' : 'bg-emerald-500'
                                                }`}></div>
                                            <span className="text-[11px] font-medium text-zinc-300 light:text-gray-700 capitalize">{member.role}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="col-span-4 flex items-center gap-1.5 overflow-hidden">
                                        {member.permissions.slice(0, 2).map((perm, i) => (
                                            <span key={i} className="px-1.5 py-0.5 rounded bg-zinc-800/40 light:bg-gray-200 text-[10px] text-zinc-400 light:text-gray-600 font-mono border border-zinc-800/50 light:border-gray-300">
                                                {perm}
                                            </span>
                                        ))}
                                        {member.permissions.length > 2 && (
                                            <span className="text-[10px] text-zinc-600 light:text-gray-400 font-medium">+{member.permissions.length - 2}</span>
                                        )}
                                    </div>

                                    {/* 4. Actions / Date */}
                                    <div className="col-span-2 flex items-center justify-end gap-3 text-right">
                                        <span className="text-xs text-zinc-600 light:text-gray-400 font-mono hidden sm:inline-block">
                                            {new Date(member.joined_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
                                        </span>
                                        <button className={`p-1.5 rounded-md transition-colors ${isExpanded ? 'bg-zinc-800 light:bg-gray-300 text-zinc-100 light:text-black' : 'text-zinc-600 light:text-gray-400 hover:text-zinc-300 light:hover:text-black hover:bg-zinc-800/50 light:hover:bg-gray-200'}`}>
                                ...
                                {isExpanded && (
                                    <div className="border-b border-zinc-800 light:border-gray-200 bg-[#0C0C0E] light:bg-gray-50 px-6 py-6 animate-in slide-in-from-top-2 duration-200">
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">

                                            {/* Col 1: Identity Card */}
                                            <div className="space-y-4">
                                                <h3 className="text-[10px] font-bold text-zinc-500 light:text-gray-500 uppercase tracking-widest">User Profile</h3>
                                                <div className="flex items-start gap-4 p-4 rounded-xl border border-zinc-800/60 light:border-gray-200 bg-zinc-900/20 light:bg-white">
                                                    <CreatorPfp
                                                        userId={member.user_id}
                                                        currentUserId={currentUserId}
                                                        currentUserPfp={currentUserPfp}
                                                    ...
                                                    <div className="space-y-1">
                                                        <div className="font-medium text-zinc-200 light:text-black">{member.full_name}</div>
                                                        <div className="text-sm text-zinc-500 light:text-gray-500 flex items-center gap-2 font-mono">
                                                            <Mail size={10} /> {member.email}
                                                        </div>

                                                        {/* PHONE NUMBER SECTION */}
                                                        <div className="text-sm text-zinc-500 light:text-gray-500 flex items-center gap-2 font-mono" title="Phone Number">
                                                        ...
                                                        >
                                                            ...
                                                            {copyStatus === member.user_id ? (
                                                                ...
                                                                <UserIcon size={10} className="text-zinc-500 light:text-gray-500" /> // Default icon
                                                            )}

                                                            {/* Text changes based on copy status */}
                                                            <span className={`truncate max-w-[120px] ${copyStatus === member.user_id ? 'text-emerald-400' : 'text-zinc-500 light:text-gray-500 group-hover:text-zinc-300 light:group-hover:text-black'}`}>
                                                                {copyStatus === member.user_id ? 'Copied!' : member.user_id}
                                                            </span>

                                                            {/* Copy Icon changes based on status */}
                                                            <Copy size={10} className={copyStatus === member.user_id ? 'text-emerald-400' : 'text-zinc-600 light:text-gray-400 group-hover:text-zinc-400 light:group-hover:text-gray-600'} />
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="space-y-4">
                                                <h3 className="text-[10px] font-bold text-zinc-500 light:text-gray-500 uppercase tracking-widest">Role & Permissions</h3>
                                                <div className="p-4 rounded-xl border border-zinc-800/60 light:border-gray-200 bg-zinc-900/20 light:bg-white space-y-3">
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span className="text-sm font-medium text-zinc-300 light:text-gray-700 capitalize">{member.role}</span>
                                                        </div>
                                                        <span className="text-[10px] px-1.5 py-0.5 bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 rounded">
                                                            Active
                                                        </span>
                                                    </div>
                                                    <div className="border-t border-zinc-800/50 light:border-gray-200 pt-3">
                                                        <div className="text-[10px] text-zinc-500 light:text-gray-500 mb-2">Effective Permissions</div>
                                                        <div className="flex flex-wrap gap-1.5">
                                                            {member.permissions.map((perm, i) => (
                                                                <span key={i} className="px-2 py-1 rounded bg-zinc-950 light:bg-gray-100 border border-zinc-800 light:border-gray-200 text-[10px] text-zinc-400 light:text-gray-600 font-mono">
                                                                    {perm}
                                                                </span>
                                                            ))}
                                                        </div>
                                            ...
                                            <div className="space-y-4">
                                                <h3 className="text-[10px] font-bold text-zinc-500 light:text-gray-500 uppercase tracking-widest">Metadata</h3>
                                                <div className="p-4 rounded-xl border border-zinc-800/60 light:border-gray-200 bg-zinc-900/20 light:bg-white flex flex-col justify-between h-auto gap-3">
                                                    <div>
                                                        <div className="flex items-center gap-2 text-xs text-zinc-400 light:text-gray-600 mb-2">
                                                            <Calendar size={12} className="text-zinc-600 light:text-gray-400" />
                                                            <span>Member since <span className="text-zinc-200 light:text-black">{new Date(member.joined_at).toLocaleDateString()}</span></span>
                                                        </div>
                                                        <div className="flex items-center gap-2 text-xs text-zinc-400 light:text-gray-600">
                                                            <ShieldAlert size={12} className="text-zinc-600 light:text-gray-400" />
                                                            <span>Last active: <span className="text-zinc-200 light:text-black">Recently</span></span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
