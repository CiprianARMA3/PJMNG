...
const getRoleBadgeStyle = (role: string = 'Viewer') => {
    switch (role.toLowerCase()) {
        case 'owner': return 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20 light:bg-yellow-100 light:text-yellow-700 light:border-yellow-200';
        case 'admin': return 'bg-blue-500/10 text-blue-400 border-blue-500/20 light:bg-blue-100 light:text-blue-700 light:border-blue-200';
        case 'editor': return 'bg-green-500/10 text-green-400 border-green-500/20 light:bg-green-100 light:text-green-700 light:border-green-200';
        default: return 'bg-white/5 text-white/40 border-white/10 light:bg-gray-100 light:text-gray-500 light:border-gray-200';
    }
};
...
export default function CalendarPage() {
    ...
    const getEventStyle = (task: Task, dayTasks: Task[]) => {
        ...
        const colorMap = {
            default: { bg: 'bg-blue-600/10 light:bg-blue-100', border: 'border-blue-500/50 light:border-blue-300', bar: 'border-l-blue-500', text: 'text-blue-100 light:text-blue-800' },
            bug: { bg: 'bg-red-600/10 light:bg-red-100', border: 'border-red-500/50 light:border-red-300', bar: 'border-l-red-500', text: 'text-red-100 light:text-red-800' },
            feature: { bg: 'bg-purple-600/10 light:bg-purple-100', border: 'border-purple-500/50 light:border-purple-300', bar: 'border-l-purple-500', text: 'text-purple-100 light:text-purple-800' },
        };
            ...
            <main className="flex-1 ml-64 flex flex-col h-full bg-[#0a0a0a] light:bg-gray-50">
                ...
                <div className="flex-none h-16 mt-[60px] px-6 border-b border-white/5 light:border-gray-200 flex items-center justify-between bg-[#0a0a0a] light:bg-white z-20">
                    <div className="flex items-center gap-6">
                        <h1 className="text-xl font-bold tracking-tight text-white light:text-gray-900">Calendar<span className="text-white/30 light:text-gray-400 text-lg font-light ml-1">Overview</span></h1>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <h2 className="text-sm font-semibold text-white/90 min-w-[140px] light:text-gray-900">
                                    ...
                                    {dateStrip.map((day, i) => {
                                        ...
                                        return (
                                            <div key={i} className="flex-1 border-r border-white/5 light:border-gray-200 flex items-center justify-center gap-2 last:border-r-0">
                                                <span className={`text-xs uppercase font-bold tracking-wider ${isToday ? 'text-purple-400' : 'text-white/40 light:text-gray-500'}`}>{day.toLocaleString('default', { weekday: 'short' })}</span>
                                                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold ${isToday ? 'bg-purple-600 text-white' : 'text-white/80 light:text-gray-800'}`}>{day.getDate()}</div>
                                            </div>
                                        );
                                    })}
                                </div>
                                ...
                                <div className="flex-1 relative" style={{ height: `${GRID_TOTAL_HEIGHT}px` }}>
                                    ...
                                    <div className="absolute inset-0 flex h-full">
                                        {dateStrip.map((day, colIndex) => {
                                            ...
                                            return (
                                                <div key={colIndex} className="flex-1 border-r border-white/5 light:border-gray-200 last:border-r-0 relative h-full group/col hover:bg-white/[0.01] light:hover:bg-gray-100 transition-colors">
                                                    {dayTasks.map(task => {
                                                        ...
                                                        `}
                                                            >
                                                                ...
                                                                {parseInt(style.height) > 80 && task.description && (
                                                                    <p className="text-[9px] text-white/50 light:text-gray-600 line-clamp-2 leading-3 mb-1 break-words">
                                                                        {task.description}
                                                                    </p>
                                                                )}
                                                                        ...
                                                                        <div className="flex -space-x-2 self-end relative">
                                                                            {attendees.slice(0, 3).map(uid => (
                                                                                <div key={uid} className="group/avatar relative">
                                                                                    <div className="w-5 h-5 rounded-full border border-[#161616] light:border-white bg-[#222] light:bg-gray-200 overflow-hidden hover:z-20 hover:scale-110 transition-transform relative z-10 cursor-help">
                                                                                    ...
                                                                                    {/* HOVER TOOLTIP */}
                                                                                    <div className="absolute bottom-full right-0 mb-1.5 hidden group-hover/avatar:flex flex-col items-end gap-1 bg-[#161616] light:bg-white border border-white/10 light:border-gray-200 p-2 rounded shadow-2xl z-50 whitespace-nowrap min-w-[100px] pointer-events-none">
                                                                                        <span className="text-xs font-bold text-white light:text-gray-900">{userMap[uid]?.name || "Unknown"}</span>
                                                                            ...
                                                                            {attendees.length > 3 && (
                                                                                <div className="w-5 h-5 rounded-full border border-[#161616] light:border-white bg-[#333] light:bg-gray-300 flex items-center justify-center text-[8px] text-white light:text-gray-700 font-bold z-0">
                                                                                    +{attendees.length - 3}
                                                                                </div>
                                                                            )}
                                                                        </div>
                            ...
                            <table className="w-full text-left text-sm text-white/70 light:text-gray-600 border-collapse">
                                ...
                                <tbody className="divide-y divide-white/5 light:divide-gray-200">
                                    {tasks.length === 0 ? (
                                        ...
                                        const tags = getTaskTags(task);
                                        return (
                                            <tr key={task.id} onClick={() => setSelectedTask(task)} className={`cursor-pointer transition-colors ${isPast ? 'opacity-50 hover:opacity-100 past-event-striped' : 'hover:bg-[#111] light:hover:bg-gray-50'}`}>
                                                ...
                                                <td className="px-4 py-3">
                                                    {task.issue ? (
                                                        <span className={`text-[10px] px-2 py-1 rounded border font-medium ${task.issue.type === 'Bug' ? 'bg-red-500/10 text-red-400 border-red-500/20 light:bg-red-100 light:text-red-700 light:border-red-200' : 'bg-purple-500/10 text-purple-400 border-purple-500/20 light:bg-purple-100 light:text-purple-700 light:border-purple-200'}`}>
                                                    ...
                                                    <div className="flex -space-x-2">
                                                        {(task.metadata?.attendees || []).map((uid, i) => (
                                                            <div key={i} className="w-8 h-8 rounded-full bg-[#222] light:bg-gray-200 border-2 border-[#1a1a1a] light:border-white overflow-hidden hover:z-10 relative transition-transform hover:scale-110" title={userMap[uid]?.name || ""}>
                                                                {userMap[uid]?.metadata?.avatar_url ? <img src={userMap[uid].metadata.avatar_url} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-[10px] text-white light:text-gray-700">{userMap[uid]?.name?.[0]}</div>}
                                                            </div>
                                                        ))}
                                                    </div>
                                                </td>
            ...
            {selectedTask && (
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-[#161616] light:bg-white border border-white/10 light:border-gray-200 rounded-xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
                        <div className="p-6 border-b border-white/10 light:border-gray-100 flex justify-between items-start bg-[#1a1a1a] light:bg-white">
                            <h3 className="font-bold text-2xl text-white light:text-gray-900 leading-tight">{selectedTask.title}</h3>
                            <button onClick={() => setSelectedTask(null)} className="p-2 bg-white/5 light:bg-gray-100 hover:bg-white/10 light:hover:bg-gray-200 rounded-lg text-white/60 light:text-gray-500 hover:text-white light:hover:text-gray-900 transition-colors"><X size={20} /></button>
                        </div>
                        ...
                        <div className="p-6 space-y-6 overflow-y-auto">
                            ...
                            {selectedTask.issue && (
                                <div className="flex items-center gap-3 p-3 bg-white/5 light:bg-gray-50 rounded-lg border border-white/10 light:border-gray-200">
                                    <div className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider border ${selectedTask.issue.type === 'Bug' ? 'bg-red-500/10 text-red-400 border-red-500/20 light:bg-red-100 light:text-red-700 light:border-red-200' : 'bg-purple-500/10 text-purple-400 border-purple-500/20 light:bg-purple-100 light:text-purple-700 light:border-purple-200'}`}>
                                        {selectedTask.issue.type}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <div className="text-[10px] text-white/40 light:text-gray-500 uppercase font-bold mb-0.5">Linked Issue</div>
                                        <div className="text-sm font-medium text-white/90 light:text-gray-800 truncate flex items-center gap-2">
                                            <LinkIcon size={14} className="text-white/40 light:text-gray-400" />
                            ...
                            <div className="flex items-center gap-4 bg-white/5 light:bg-gray-50 p-4 rounded-xl border border-white/5 light:border-gray-200">
                                <div className="p-3 bg-[#0a0a0a] light:bg-white rounded-lg border border-white/5 light:border-gray-200 text-center min-w-[70px]">
                                    <div className="text-[10px] uppercase font-bold text-white/40 light:text-gray-500">{new Date(selectedTask.task_date).toLocaleString('default', { month: 'short' })}</div>
                                    <div className="text-2xl font-bold text-white light:text-gray-900">{new Date(selectedTask.task_date).getDate()}</div>
                                </div>
                                <div className="space-y-1">
                                    <div className="flex items-center gap-2 text-white/90 light:text-gray-900 text-base font-bold">
                                        <Clock size={16} className="text-white/40 light:text-gray-400" />
                                        {selectedTask.start_time.slice(0, 5)} - {selectedTask.end_time.slice(0, 5)}
                                    </div>
                                    <div className="text-sm text-white/50 light:text-gray-500 flex items-center gap-2">
                                        ...
                                        <div className="flex items-center gap-1.5 px-2 py-0.5 bg-[#0a0a0a] light:bg-white rounded-full border border-white/5 light:border-gray-200">
                                            <div className="w-4 h-4 rounded-full bg-[#222] light:bg-gray-200 overflow-hidden">
                                                {(userMap[selectedTask.creator_id || '']?.metadata?.avatar_url || selectedTask.creator?.metadata?.avatar_url) ?
                                                    ...
                                                    <div className="w-full h-full flex items-center justify-center text-[8px] text-white light:text-gray-700">{(userMap[selectedTask.creator_id || '']?.name?.[0] || selectedTask.creator?.name?.[0])}</div>
                                                }
                                            </div>
                                            <span className="text-xs font-medium text-white/80 light:text-gray-800">{userMap[selectedTask.creator_id || '']?.name || selectedTask.creator?.name || "Unknown"}</span>
                            ...
                            {selectedTask.description && (
                                <div className="space-y-2">
                                    <label className="text-[10px] uppercase font-bold text-white/40 light:text-gray-500">Description</label>
                                    <div className="text-sm text-white/70 light:text-gray-700 leading-relaxed bg-white/5 light:bg-gray-50 p-4 rounded-lg border border-white/5 light:border-gray-200 whitespace-pre-wrap">
                                        {selectedTask.description}
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="pt-4 border-t border-white/5 light:border-gray-200">
                                <label className="text-[10px] uppercase font-bold text-white/40 light:text-gray-500 block mb-3">Attendees ({selectedTask.metadata?.attendees?.length || 0})</label>
                                <div className="grid grid-cols-1 gap-2">
                                    {(selectedTask.metadata?.attendees || []).map((uid) => (
                                        <div key={uid} className="flex items-center gap-3 p-2 pr-3 rounded-lg bg-white/5 light:bg-gray-50 border border-white/5 light:border-gray-200 hover:bg-white/10 light:hover:bg-gray-100 transition-colors cursor-default">
                                            <div className="w-8 h-8 rounded-full bg-[#222] light:bg-gray-200 border border-white/10 light:border-gray-300 overflow-hidden shrink-0">
                                                {userMap[uid]?.metadata?.avatar_url ? <img src={userMap[uid].metadata.avatar_url} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-[10px] text-white light:text-gray-700">{userMap[uid]?.name?.[0]}</div>}
                                            </div>
                                            <div className="flex flex-col min-w-0">
                                                <span className="text-xs font-bold text-white/90 light:text-gray-900 truncate">{userMap[uid]?.name}</span>
                                                <div className="flex">
                                                    {/* --- ROLE BADGE --- */}
                                    ...
                                    {(selectedTask.metadata?.attendees || []).length === 0 && <span className="text-sm text-white/30 light:text-gray-400 italic">No attendees have joined this event yet.</span>}
                                </div>
                            </div>
                        </div>
                    </div>
