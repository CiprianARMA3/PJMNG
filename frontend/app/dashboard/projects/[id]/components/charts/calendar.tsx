...
export default function ProjectCalendar() {
    ...
    const getEventStyle = (task: Task, dayTasks: Task[]) => {
        ...
        const colorMap: any = {
            purple: { bg: 'bg-purple-600/10 light:bg-purple-100', border: 'border-purple-500 light:border-purple-400', text: 'text-purple-100 light:text-purple-800' },
            red: { bg: 'bg-red-600/10 light:bg-red-100', border: 'border-red-500 light:border-red-400', text: 'text-red-100 light:text-red-800' },
            green: { bg: 'bg-green-600/10 light:bg-green-100', border: 'border-green-500 light:border-green-400', text: 'text-green-100 light:text-green-800' },
        };
    ...
    return (
        <div className="h-[600px] bg-[#0a0a0a] light:bg-white light:border light:border-gray-200 rounded-xl flex flex-col overflow-hidden font-sans shadow-inner relative">
            ...
            @media (prefers-color-scheme: light) {
                .widget-scrollbar::-webkit-scrollbar-thumb { background: #d1d5db; }
                .widget-scrollbar::-webkit-scrollbar-thumb:hover { background: #9ca3af; }
            }
        ...
        `}</style>
            ...
            <div className="flex-none h-12 px-4 border-b border-white/5 light:border-gray-200 flex items-center justify-between bg-[#0a0a0a] light:bg-white">
                <div className="flex items-center gap-3">
                    <div className="flex items-center bg-[#161616] light:bg-gray-100 rounded-md border border-white/5 light:border-gray-200 p-0.5">
                        <button onClick={() => navigateDate(-1)} className="p-1 hover:bg-white/10 light:hover:bg-gray-200 rounded text-white/60 light:text-gray-500"><ChevronLeft size={14} /></button>
                        <button onClick={() => setStartDate(getStartOfWeek(new Date()))} className="px-2 text-[10px] font-bold uppercase text-white/60 light:text-gray-600 hover:text-white light:hover:text-black">Today</button>
                        <button onClick={() => navigateDate(1)} className="p-1 hover:bg-white/10 light:hover:bg-gray-200 rounded text-white/60 light:text-gray-500"><ChevronRight size={14} /></button>
                    </div>
                ...
                <div className="flex items-center gap-2">
                    {viewMode === 'calendar' && (
                        <div className="flex bg-[#161616] light:bg-gray-100 rounded-md border border-white/5 light:border-gray-200 p-0.5">
                            <button onClick={() => setHourHeight(60)} className={`p-1.5 rounded text-white/60 light:text-gray-500 hover:text-white light:hover:text-black ${hourHeight === 60 ? 'bg-white/10 light:bg-white light:shadow-sm' : ''}`}><Minimize2 size={14} /></button>
                            <button onClick={() => setHourHeight(120)} className={`p-1.5 rounded text-white/60 light:text-gray-500 hover:text-white light:hover:text-black ${hourHeight === 120 ? 'bg-white/10 light:bg-white light:shadow-sm' : ''}`}><Maximize2 size={14} /></button>
                        </div>
                    )}
                    {viewMode === 'calendar' && (
                        <div className="hidden sm:flex bg-[#161616] light:bg-gray-100 rounded-md border border-white/5 light:border-gray-200 p-0.5">
                            {[1, 3, 5, 7].map(n => (
                                <button key={n} onClick={() => setNumDaysInView(n)} className={`px-2 py-1 rounded text-[9px] font-bold uppercase transition-all ${numDaysInView === n ? 'bg-white/10 light:bg-white light:shadow-sm text-white light:text-black' : 'text-white/40 light:text-gray-500 hover:text-white light:hover:text-black'}`}>{n}D</button>
                            ))}
                        </div>
                    )}
                    <div className="flex bg-[#161616] light:bg-gray-100 rounded-md border border-white/5 light:border-gray-200 p-0.5">
            ...
            {viewMode === 'calendar' && (
                ...
                <div className="flex-1 overflow-y-auto widget-scrollbar bg-[#0a0a0a] light:bg-white relative flex flex-col">
                            ...
                            {dateStrip.map((day, i) => {
                                ...
                                return (
                                    <div key={i} className="flex-1 border-r border-white/5 light:border-gray-200 flex items-center justify-center gap-1.5 last:border-r-0">
                                        <span className={`text-[9px] uppercase font-bold tracking-wider ${isToday ? 'text-purple-400' : 'text-white/40 light:text-gray-500'}`}>{day.toLocaleString('default', { weekday: 'short' })}</span>
                                        <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${isToday ? 'bg-purple-600 text-white' : 'text-white/80 light:text-gray-800'}`}>{day.getDate()}</div>
                                    </div>
                                );
                            })}
                        </div>
                ...
                <div className="flex-1 overflow-auto widget-scrollbar p-2">
                    <table className="w-full text-left text-xs text-white/70 light:text-gray-600 border-collapse">
                        ...
                        <tbody className="divide-y divide-white/5 light:divide-gray-200">
                            {tasks.length === 0 ? (
                                <tr><td colSpan={4} className="text-center py-8 text-white/20 light:text-gray-400 italic">No events found</td></tr>
                                ...
                                return (
                                    <tr key={task.id} onClick={() => setSelectedTask(task)} className={`cursor-pointer transition-colors ${isPast ? 'opacity-50 hover:opacity-100 past-event-striped' : 'hover:bg-[#111] light:hover:bg-gray-50'}`}>
                                        <td className="px-3 py-2 font-mono text-[10px] text-white/50 light:text-gray-500">
                                            <div className="text-white/80 light:text-gray-900">{new Date(task.task_date).toLocaleDateString()}</div>
                                        ...
                                        <td className="px-3 py-2">
                                            <div className="flex gap-1 flex-wrap">
                                                ...
                                                {tags.length === 0 && <span className="text-[9px] text-white/10 light:text-gray-300">-</span>}
                                            </div>
                                        </td>
                                        <td className="px-3 py-2">
                                            <div className="flex -space-x-1.5">
                                                {(task.metadata?.attendees || []).map((uid, i) => (
                                                    <div key={i} className="w-5 h-5 rounded-full bg-[#222] light:bg-gray-200 border border-[#333] light:border-gray-300 overflow-hidden" title={userMap[uid]?.name || ""}>
                                                        {userMap[uid]?.metadata?.avatar_url ? <img src={userMap[uid].metadata.avatar_url} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-[8px] text-gray-500">{userMap[uid]?.name?.[0]}</div>}
                                                    </div>
                                                ))}
                                            </div>
                                        </td>
            ...
            {selectedTask && (
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-[#161616] light:bg-white border border-white/10 light:border-gray-200 rounded-xl w-full max-w-sm overflow-hidden shadow-2xl flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
                        <div className="p-4 border-b border-white/10 light:border-gray-200 flex justify-between items-start bg-[#1a1a1a] light:bg-gray-50">
                            <div>
                                {selectedTask.issue && (
                                ...
                                <h3 className="font-bold text-lg text-white light:text-black leading-tight">{selectedTask.title}</h3>
                            </div>
                            <button onClick={() => setSelectedTask(null)} className="p-1.5 bg-white/5 light:bg-gray-200 hover:bg-white/10 light:hover:bg-gray-300 rounded text-white/60 light:text-gray-600 hover:text-white light:hover:text-black"><X size={16} /></button>
                        </div>
                        ...
                        <div className="p-4 space-y-4 overflow-y-auto widget-scrollbar">
                            ...
                            <div className="flex items-center gap-3">
                                <div className="p-2 bg-white/5 light:bg-gray-100 rounded border border-white/5 light:border-gray-200 text-center min-w-[50px]">
                                    <div className="text-[9px] uppercase font-bold text-white/40 light:text-gray-500">{new Date(selectedTask.task_date).toLocaleString('default', { month: 'short' })}</div>
                                    <div className="text-lg font-bold text-white light:text-black">{new Date(selectedTask.task_date).getDate()}</div>
                                </div>
                                <div className="space-y-0.5">
                                    <div className="flex items-center gap-1.5 text-white/80 light:text-gray-800 text-sm font-medium">
                                        <Clock size={14} className="text-white/40 light:text-gray-400" />
                                        {selectedTask.start_time.slice(0, 5)} - {selectedTask.end_time.slice(0, 5)}
                                    </div>
                                    <div className="text-xs text-white/40 light:text-gray-500">Created by {selectedTask.creator?.name || "Unknown"}</div>
                                </div>
                            </div>

                            {selectedTask.description && (
                                <div className="text-xs text-white/70 light:text-gray-600 leading-relaxed bg-white/5 light:bg-gray-50 p-3 rounded border border-white/5 light:border-gray-200">
                                    {selectedTask.description}
                                </div>
                            )}
                            ...
                            <div>
                                <label className="text-[9px] uppercase font-bold text-white/40 light:text-gray-500 block mb-2">Attendees ({selectedTask.metadata?.attendees?.length || 0})</label>
                                <div className="flex flex-wrap gap-2">
                                    {(selectedTask.metadata?.attendees || []).map((uid) => (
                                        <div key={uid} className="flex items-center gap-1.5 p-1.5 rounded bg-white/5 light:bg-gray-100 border border-white/5 light:border-gray-200">
                                            <div className="w-5 h-5 rounded-full bg-[#222] light:bg-gray-200 border border-white/10 light:border-gray-300 overflow-hidden">
                                                {userMap[uid]?.metadata?.avatar_url ? <img src={userMap[uid].metadata.avatar_url} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-[8px] text-gray-500">{userMap[uid]?.name?.[0]}</div>}
                                            </div>
                                            <span className="text-[10px] text-white/70 light:text-gray-700">{userMap[uid]?.name}</span>
                                        </div>
                                    ))}
                                    {(selectedTask.metadata?.attendees || []).length === 0 && <span className="text-xs text-white/20 light:text-gray-400 italic">None</span>}
                                </div>
                            </div>
                        </div>
                    </div>
